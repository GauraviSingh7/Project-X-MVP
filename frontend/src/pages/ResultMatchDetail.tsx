...
function deriveResult(match: any) {
  const [first, second] = match.scorecard;

  const r1 = parseInt(first.score.split("/")[0], 10);
  const r2 = parseInt(second.score.split("/")[0], 10);
  const w2 = parseInt(second.score.split("/")[1], 10);

  if (r2 > r1) {
    return `Team 2 won by ${10 - w2} wickets`;
  }

  return `Team 1 won by ${r1 - r2} runs`;
}
...
export default function ResultMatchDetail() {
  ...
  if (isLoading) return <LoadingState message="Loading match result..." />;
  ...
  return (
    <>
      <Helmet>
        <title>Match Result | STRYKER</title>
      </Helmet>
      ...
      <div className="container-content py-8 space-y-6">
          ...
          {[inn1, inn2].map((inning: any, idx: number) => (
            <div key={idx} className="bg-card border rounded-lg p-4">
              <p className="text-sm text-muted-foreground mb-1">
                Team {idx + 1} — Inning {inning.inning_number}
              </p>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <LineupSection
            title="Team 1 Playing XI"
            players={match.lineups.home}
          />
          <LineupSection
            title="Team 2 Playing XI"
        ...
        <section className="bg-card border rounded-lg p-5 text-sm space-y-2 text-muted-foreground">
          ...
          {match.toss && (
            <div className="flex items-center gap-2">
              ...
              Toss won by Team {match.toss.won_by_team_id === inn1.team_id ? "1" : "2"}, elected to{" "}
              {match.toss.elected}
            </div>
          )}
        </section>
